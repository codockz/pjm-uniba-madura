...
@section('content')
            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">Data Tupoksi PJM</h3>
                    </div>
                    <div class="card-body">
                        <button type="button" class="btn btn-primary btn-sm mb-2" data-toggle="modal"
                            data-target="#exampleModal">
                            + Tambah Data Tupoksi PJM
                        </button>
                        <button type="button" class="btn btn-primary btn-sm mb-2" data-toggle="modal"
                            data-target="#kategori_tupoksi">
                            + Tambah Data Kategori Tupoksi PJM
                        </button>
                        <button type="button" class="btn btn-primary btn-sm mb-2" data-toggle="modal"
                            data-target="#modal-xl">
                             Data Kategori Tupoksi PJM
                        </button>
    ...
    <div class="modal fade" id="kategori_tupoksi" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="add_form_kategori_tupoksi_pjm" class="row">
                        @csrf
                        <div class="form-group col-12">
                            <label for="kategori_tupoksi_pjm">Kategori Tupoksi PJM</label>
                            <input name="kategori_tupoksi_pjm" id="kategori_tupoksi_pjm" class="form-control">
                            <div class="invalid-feedback" id="kategori_tupoksi_pjm-error">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="add_form" class="row">
                    @csrf
                    <div class="form-group col-12">
                        <label for="kategori_pjm">Kategori PJM</label>
                        <select class="form-control select_kategori_id" name="kategori_pjm" id="select_kategori_id">
                            <option selected disabled>-- Pilih Kategori --</option>
                            @foreach ($kategori as $x)
                            <option value="{{ $x->id }}">{{ $x->nama_kategori }}</option>
                            @endforeach
                        </select>
                        <div class="invalid-feedback" id="kategori_pjm-error">

                        </div>
                    </div>
                    <div class="form-group col-12">
                        <label for="isi_tupoksi_pjm">Isi Tupoksi PJM</label>
                        <textarea name="isi_tupoksi_pjm" id="isi_tupoksi_pjm" rows="3" class="form-control"></textarea>
                        <div class="invalid-feedback" id="isi_tupoksi_pjm-error">
...
<div class="modal fade" id="modal-xl">
    ...
    <div class="modal-content">
    <div class="modal-header">
    <h4 class="modal-title">Data Kategori Tupoksi</h4>
    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
    <span aria-hidden="true">&times;</span>
    </button>
    </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            var newTable = $('#example1').DataTable({
                ...
                columns: [{
                    ...
                    {
                        data: 'kategori_tupoksi',
                        name: 'kategori_tupoksi'
                    },
                    {
                        data: 'isi_tupoksi',
                        name: 'isi_tupoksi'
        ...
        $(document).ready(function() {
            ...
            var newTable = $('#example2').DataTable({
                ...
                type: 'post',
                ajax: {
                    url: "{{ route('kategori_tupoksi_pjm.index') }}",
                },
                columns: [{
                        data: 'id',
                        render: function(data, type, row, meta) {
        ...
        $("#add_form").submit(function(e) {
            ...
            var url = "{{ route('tupoksi_pjm.store') }}";
        ...
        $("#add_form_kategori_tupoksi_pjm").submit(function(e) {
            e.preventDefault();
            var formData = new FormData(this);
            var url = "{{ route('kategori_tupoksi_pjm.store') }}";
            $('#btn_add').prop('disabled', true);
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.status == 400) {
                        ...
                        $('#btn_add').prop('disabled', false);
                        $('#kategori_tupoksi').modal('hide');
                        $("#add_form_kategori_tupoksi_pjm")[0].reset(); // Fix here
        ...
        $("#add_new_kategori").on("click", '.btnSaveKategori', function(e) {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('kategori_tupoksi_pjm.updated') }}",
                dataType: "json",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
        ...
        function deleteDataKategori(id) {
            ...
            swalWithBootstrapButtons.fire({
                ...
                if (result.value) {
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('kategori_tupoksi_pjm.delete') }}",
                            type: "POST",
                            data: {
                                ids: id,
                            },
